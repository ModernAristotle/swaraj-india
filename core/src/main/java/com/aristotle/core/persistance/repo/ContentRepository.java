...
public interface ContentRepository extends JpaRepository<Content, Long> {
    ...
    //Web
    @Query("select content from Content content where content.contentType=?1 and content.global=true order by publishDate desc")
    public List<Content> getGlobalContent(String contentType, Pageable pageable);

    @Query("select content from Content content join content.locations locations where locations.id in ?2 and content.contentStatus='Published' and content.contentType=?1 order by content.publishDate desc")
    ...
    @Query("select content from Content content where content.contentType=?1 and content.global=true and content.contentStatus='Published' order by content.publishDate desc")
    ...
    @Query("select content from Content content join content.locations locations where locations.id in ?1 and content.contentStatus='Published' and content.contentType in ('News', 'PressRelease') order by content.publishDate desc")
    public List<Content> getLatestLocationPublishedContent(Set<Long> locationIds, Pageable pageable);

    @Query("select content from Content content where content.contentStatus='Published' and content.contentType in ('News', 'PressRelease') order by content.publishDate desc")
